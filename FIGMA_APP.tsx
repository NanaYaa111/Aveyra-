...
const NAV_ITEMS: { id: Screen; label: string; Icon: React.ComponentType<IcoProps> }[] = [
  ...
  return (
    <aside className="hidden md:flex flex-col w-60 shrink-0 h-full border-r"
      ...
      <div className="px-6 pt-8 pb-6 flex items-center gap-2">
        <span style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, color: 'var(--color-ink)', letterSpacing: '-0.01em' }}>
          Aveyra
        </span>
      ...
      <div className="px-6 py-6">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-ink)' }}>Jordan</div>
            <div style={{ fontSize: 11, color: 'var(--color-muted)' }}>your person</div>
          </div>
        </div>
      </div>
    </aside>
...
function MobileNav({ screen, onNav }: { screen: Screen; onNav: (s: Screen) => void }) {
  return (
    <nav className="md:hidden fixed bottom-0 inset-x-0 z-50 h-16 border-t"
        ...
        {NAV_ITEMS.map(({ id, label, Icon }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => onNav(id)}
              className="flex flex-col items-center justify-center gap-0.5 h-full shrink-0"
              style={{
                width: 56,
                color: active ? 'var(--color-accent)' : 'var(--color-muted)',
              }}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center">
            <span style={{ fontSize: 12, color: 'var(--color-muted)' }}>
              Jordan hasn't answered yet either.
            </span>
        ...
        {jordanCheckedIn ? (
          ...
          <div>
            <div style={{ fontSize: 14, color: 'var(--color-muted)', fontStyle: 'italic' }}>
              Jordan hasn't checked in yet today.
            </div>
          </div>
        )}
      </Card>
    ...
    <div className="max-w-[640px] mx-auto px-5 py-8 md:py-12">
      ...
      {state === 'waiting' && (
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3 py-4">
            <div className="flex-1 h-px" style={{ backgroundColor: 'var(--color-edge)' }} />
            <span style={{ fontSize: 14, color: 'var(--color-muted)', fontStyle: 'italic' }}>Waiting to hear what Jordan makes of it…</span>
            <div className="flex-1 h-px" style={{ backgroundColor: 'var(--color-edge)' }} />
          </div>
        </div>
      )}
...
export default function App() {
  ...
  return (
    <div className="flex md:flex-row h-dvh overflow-hidden"
      style={{ backgroundColor: 'var(--color-ground)' }}>

      <DesktopNav screen={screen} onNav={setScreen} />

      <main className={isFullHeight
